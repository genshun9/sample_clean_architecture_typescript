...
export class PostAggregate extends Aggregate<Post, PostID, string> {
    ...
    toDTO() {
        return {
            id: this.getRoot().getID().getValue(),
            message: this.getRoot().getMessage().getValue(),
            favorites: {
                count: this.getFavoriteCount(),
                userIDs: this.getFavoriteUserIDs().map(id => id.getValue())
            },
            createdAt: this.getRoot().getCreatedAt()
        };
    }
}